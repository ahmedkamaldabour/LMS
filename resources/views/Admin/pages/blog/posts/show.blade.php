...
@push('css')
        ...
        <link rel="stylesheet" href="{{asset("assetsAdmin/src/assets/css/message.css")}}">
@endpush

@section('content')
    <div class="message d-none" id="message">
        <div class="parent">
            @foreach($post->comments as $comment)
                <div class="comments">
                    <p class="text-danger">{{$comment->created_at->diffForHumans()}}</p>
                    <div class="msg">
                        <p class="mx-4">{{$comment->body}}</p>
                        @if( $comment->status == 0 )
                            <form action="{{route('admin.posts.update_status',$comment)}}" method="post">
                                @csrf @method('PUT')
                                <button class="btnmsg">
                                    <i class="fa-solid fa-check text-success fs-2 mx-4"></i>
                                </button>
                            </form>
                        @else
                            <form action="{{route('admin.posts.update_status',$comment)}}" method="post">
                                @csrf @method('PUT')
                                <button class="btnmsg">
                                    <i class="fa-solid fa-xmark text-danger fs-2 mx-4"></i>
                                </button>
                            </form>
                        @endif
                    </div>
            ...
            @endforeach
            <button id="close" class="btn btn-primary mx-3">{{ __('actions.close') }}</button>
        </div>
    </div>
                ...
                <div class="breadcrumb-wrapper-content  mb-5 mt-3 d-flex justify-content-start bg-light">
                    <nav class="breadcrumb-style-one" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><a href="{{route('admin.posts.index')}}">{{__("posts.index")}}</a></li>
                            <li class="breadcrumb-item active" aria-current="page">{{__("posts.show_post")}}</li>
                            ...
                            <div class="featured-image">
                                ...
                                <div class="featured-image-overlay">
                                    @if(is_null($post->image))
                                        <img class="w-100" src="{{asset("assetsAdmin/src/assets/img/demo.jpg")}}" alt="profile">
                                    @else
                                        <img class="w-100" src="{{asset("images/posts/".$post->image)}}">
                                    @endif
                                </div>
                                ...
                                <div class="post-header">
                                    ...
                                    <div class="post-meta-info d-flex justify-content-between">
                                        ...
                                        <div class="align-self-center">
                                            <button id="comment" class="btn btn-success btn-icon btn-share btn-rounded">
